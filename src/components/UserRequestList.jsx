    ...
    <ul style={{ marginTop: 24 }}>
      {filtered.map(req => (
        <li key={req.id} style={{ marginBottom: 18, background: "#f7f7fa", borderRadius: 6, padding: "12px 16px" }}>
          <Link to={`/app/${appId}/user-requests/${req.id}`} style={{ textDecoration: "none", color: "inherit" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
              <div style={{ textAlign: "center", minWidth: 56 }}>
                ...
                <Button size="small" variant="primary" style={{ marginTop: 4 }} onClick={e => { e.preventDefault(); voteUserRequest(req.id); }}>
                  ↑
                </Button>
              </div>
              <div style={{ flex: 1 }}>
